//! Inter prediction (spec §8.5.2): subpel interpolation filter coefficient tables and
//! motion-compensation constants. Split out of `prob_tables.rs` (W5) since these are specific
//! to `predict::predict_inter`, not general syntax-element probability/tree tables. Doc-hidden
//! internal module, same convention as the rest of `src/` (see `lib.rs`).
