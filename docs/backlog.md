...
- **Invalid-input robustness**: a deterministic fuzz (`tests/robustness_test.rs` -- truncation +
  bit-corruption + random, both debug and release) now guards against panics on malformed input.
  It found and fixed two crash bugs (a missing-DPB-reference and a `BLOCK_INVALID` chroma block
  size, both now rejected as a `TileError` in `decode_block`). Still optional for deeper coverage:
  the libvpx `invalid-*` vector family (not hosted -- generated by libvpx tooling) and
