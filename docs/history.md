...
This is a historical record of how the decoder was built, milestone by milestone (M1
through M3 second half). It's kept for narrative/archaeology value; it is **not** updated
to reflect later changes. For the decoder's current architecture, status, and test
instructions, see [README.md](../README.md). For dated, append-only design decisions and
fixes (including corrections to claims made in this file), see
[implementation-notes.md](implementation-notes.md) — in particular its "Current state
index" section at the top, which points to the entry that supersedes each topic below.

Bracketed notes like `[superseded by ... ]` were added after the fact to flag places where
a claim below turned out to be incomplete or was later changed; the surrounding text is
...
| M4 | Full pass of VP9 conformance test vectors (expand beyond the 2 local vectors to the official vector set) | Superseded — see [implementation-notes.md](implementation-notes.md)'s "Conformance coverage" entry and the Wave 1-6 design-debt cleanup; 5 official vectors now pass bit-exact |

`decode_keyframe()` (in `src/lib.rs`) decoded a key frame all the way through, producing a
YUV420 `Frame` with the loop filter applied and cropped to display size. To decode multiple
frames in sequence (including inter frames), `Decoder::decode_frame()` was used (see the "M3
second half" section below). For our two local test vectors, we confirmed that the decoded
result of **every displayed frame** (the Y->U->V concatenated I420 byte string) had an MD5
that exactly matched the official `.ivf.md5` distributed by libvpx (`tests/conformance_test.rs`,
details below. `vp90-2-12-droppable_1`: 99/99 frames matched, `vp90-2-09-subpixel-00`: 20/20
frames matched).

[`decode_keyframe()` was removed in Wave 2b's public API redesign, and `Decoder::decode_frame()`'s
return type changed from `Result<Option<Frame>, DecodeError>` to
`Result<Vec<DecodedFrame>, DecodeError>` — see implementation-notes.md's Wave 2b entry and
README.md's "Current architecture" section for the current API.]
...
### Known limitations (at the M2 stage)

- Only 8-bit (`BitDepth == 8`) is supported. Since `Plane` is fixed at `u8`, 10-bit/12-bit
  frames cause `decode_keyframe` to return `DecodeError::UnsupportedBitDepth`. [Still true
  today — see README.md's Status/limitations table.]
- Segmentation (segment-id decoding including temporal prediction, and the
  ...
  official vector. [This text was actually written after segmentation/intra-only support landed,
  well past the original M2 milestone; kept here verbatim since it was part of the pre-Wave-6
  README's M2 section.]
...
- `src/md5.rs` (new): a from-scratch MD5 (RFC 1321) implementation (per the zero-dependency
  ...
  the values listed in RFC 1321). [`src/md5.rs` was later relocated to `tests/common/md5.rs`
  in Wave 3 — its only consumer was the test suite, so it moved out of the shipped library.]
...
- `src/header.rs`: Fully implemented the non-key-frame branch of `uncompressed_header()`
  ...
  the value from the most recent key frame / intra-only frame. [This `&[(u32,u32); ...]`
  loose-parameter style, and the `color_config` "fabricate a placeholder" pattern, were both
  replaced in Wave 2a (`PersistentState`) and Wave 2a's "honest `color_config`"
  (`Option<ColorConfig>`) respectively.]
- `src/prob_tables.rs`: Added inter-related trees (`INTER_MODE_TREE`/`INTERP_FILTER_TREE`/
  ...
  MB_MODE_COUNT=14 range, and all inter modes map to `DCT_DCT`). [`MV_REF_BLOCKS`/
  `MODE_2_COUNTER`/`COUNTER_TO_CONTEXT`/`IDX_N_COLUMN_TO_SUBBLOCK` later moved out of
  `prob_tables.rs` into `src/mv_ref_tables.rs` in Wave 5.]
- `src/compressed_header.rs`: Implemented `read_inter_mode_probs`/`read_interp_filter_probs`/
  ...
  existing API) remains as a thin wrapper around it. [`parse_compressed_header_ex()` and the
  key-frame-only wrapper were collapsed into one 3-argument `parse_compressed_header()` in
  Wave 2a.]
- `src/tile.rs`: Fully implemented `inter_frame_mode_info()` (spec section 6.4.11) and
  ...
  `KF_PARTITION_PROBS`/`partition_probs` depending on `FrameIsIntra`. [`src/mv.rs`,
  `tile.rs` itself, and `residual()` were all reshaped in Wave 5: `tile.rs` split into
  `tile/{mode_info,ref_ctx,mv_pred,residual}.rs`, and `mv.rs` was absorbed into
  `tile/mv_pred.rs` and deleted. Also, the coefficient EOB-branch counting described here
  had a bug, fixed 2026-07-14 — see implementation-notes.md.]
...
- `src/lib.rs`: Introduced a new `Decoder` that holds cross-frame state (reference frame slot
  ...
  as-is for backward compatibility. [`decode_keyframe()` was removed in Wave 2b.]
...
- `src/predict.rs`: `predict_inter()` (spec section 8.5.2), replacing `predict_inter_stub()`.
  ...
  - The filter coefficient table `SUBPEL_FILTERS` (`src/prob_tables.rs`) is transcribed
    ...
    `EIGHTTAP_SHARP`/`BILINEAR`). [`SUBPEL_FILTERS` and related constants later moved to
    `src/subpel.rs` in Wave 5.]
- `src/dpb.rs` (new): `Dpb`/`RefFrameData`. An 8-slot reference frame buffer (spec section
  ...
  implemented generically per the spec formula). [The scaling-bound arithmetic (the largest
  intermediate buffer height a scaled reference can require) was later derived and hardened in
  Wave 4b (`MAX_INTERMEDIATE_HEIGHT`) — ref-scaling is still unverified end-to-end against a
  real scaled-reference vector; see README.md's Status/limitations table.]
- `src/counts.rs` (new): probability adaptation (spec section 8.4).
  ...
  - **A note on the "special case" for `more_coefs` (end of spec section 9.3.4)**: the spec
    ...
    the comment in `src/tile.rs::tokens_and_reconstruct`). [This turned out to be wrong —
    it over-counts the EOB continue-branch and corrupts adapted coefficient probabilities in
    a way the original two test vectors happened not to exercise. Fixed 2026-07-14; see
    implementation-notes.md's "Coefficient EOB-branch over-counting" entry for the full
    diagnosis. The "empirically supported" claim above was only ever confirming the bug
    didn't hurt those two vectors, not that it was correct.]
...
- `src/lib.rs`: Added the DPB (`dpb: Dpb`), loop filter deltas (`loop_filter_deltas`), and
  ...
  [This signature was replaced again in Wave 2b: `decode_frame()` now returns
  `Result<Vec<DecodedFrame>, DecodeError>`, one element per constituent frame of a chunk
  (superframes yield several), and `decode_keyframe()`/`DecodeOutcome` are both gone.]
...
case of a mismatch) is recorded below, since it turned out to still be useful when a real
mismatch showed up later (`vp90-2-16-intra-only`'s inter frames, diagnosed via the
per-frame-diff approach in step 1 below — see implementation-notes.md's "Coefficient
EOB-branch over-counting" entry).
...
## Old test-file layout (superseded by Wave 2a/2b/3)

The original test suite (before the Wave 2a/2b/3 API and test-layer changes) looked like
this:
...
`conformance_test.rs`; this test only checks that the bitstream can be fully read.

[This whole layout is gone. `tests/header_test.rs` + `tests/compressed_header_test.rs` +
`tests/decode_test.rs` were merged verbatim into `tests/api_test.rs` in Wave 3 (their
`check_vector` helpers renamed to avoid collisions in one file, per implementation-notes.md's
Wave 3 entry). `tests/inter_frame_test.rs` was deleted in Wave 3: its "every frame parses
without panicking" check was strictly subsumed by `conformance_test.rs`'s all-frames MD5
check, which also decodes every frame in order and additionally verifies pixel-exact output.
`tests/conformance_test.rs`'s `*_first_keyframe_matches_official_md5` tests were deleted in
Wave 2b (subsumed by `*_all_frames_match_official_md5`, since frame 1 is the first line those
tests already compare). `decode_keyframe()` itself -- named throughout this section -- was
deleted in Wave 2b; see README.md's "Current architecture" section for the current
`Decoder::decode_frame()` API. See README.md's "Tests & verification" section for the
current test-file layout.]

## PNG dump (for visual inspection) — historical usage notes
...
links against it. This later grew into the automated `synthetic_streams_cross_decode_against_ffmpeg`
test described in implementation-notes.md and README.md's "Tests & verification" section.

## Original vector-download steps (superseded by scripts/fetch-vectors.{sh,ps1})

The two original local test vectors were fetched with plain `curl`/`Invoke-WebRequest`
commands, and the three WebM-only vectors additionally needed a manual remux step via
`examples/webm_to_ivf.rs`. Both are now handled by `scripts/fetch-vectors.sh` /
`scripts/fetch-vectors.ps1` (see README.md), which wrap the same download-then-remux steps
for the full manifest of 5 vectors this project uses. The original manual commands are kept
here for reference:
...
(On PowerShell, `Invoke-WebRequest -Uri <URL> -OutFile <path>` was used instead.)
...
Hidden frames with `show_frame == 0` have no line.
