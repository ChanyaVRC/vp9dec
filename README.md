...
- **`tile` + `tile/{mode_info, ref_ctx, mv_pred, residual}`** -- the tile-decode hub:
  ...
  `tile::decode_tiles_parallel` decodes a multi-tile-column frame (with exactly one tile row)
